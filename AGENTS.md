...
3. Обновить ссылку в футере главы $N-1$ с `Скоро` на `Глава N (...) →`.
4. Создать функцию `build_chapterN_html(chapters)`:
   - **Hero-блок:** `hero-tag`, `hero-title`, `hero-desc`, `hero-stats` (4 карточки метрик).
   ...
   - **Футер:** блок поздравления с кнопкой возврата на главу $N-1$ и ссылкой на следующую главу $N+1$ (Скоро).
